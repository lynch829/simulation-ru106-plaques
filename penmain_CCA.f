...
C  ----  Impact detectors (up to NIDM different detectors).
     ...
     1  LDIP(NIDM,NIDCM,3)
      DIMENSION EDIL(NIDM),EDIU(NIDM),BDIE(NIDM),RBDIE(NIDM),NDICH(NIDM)
      DIMENSION EDILL(NIDM),EDIUL(NIDM),BDIEL(NIDM),RBDIEL(NIDM)
      DIMENSION KKDI(NIDM,3)
...
C  ----  Energy-deposition detectors (up to NEDM different detectors).
      ...
      DIMENSION EDEL(NEDM),EDEU(NEDM),BDEE(NEDM),RBDEE(NEDM),NDECH(NEDM)
      DIMENSION EDELL(NEDM),EDEUL(NEDM),BDEEL(NEDM),RBDEEL(NEDM)
      DIMENSION KBDE(NB)
C
C  ----  Dose distribution (up to NDXM*NDYM*NDZM volume bins).
     ...
     1          DOSEP(NDXM,NDYM,NDZM),LDOSE(NDXM,NDYM,NDZM)
      ...
      DIMENSION DXL(3),DXU(3),BDOSE(3),RBDOSE(3),NDB(3)
C
      EXTERNAL RAND
C
      DO I=1,NSEBM
...
C
   ...
   93         CONTINUE
              KODE=0
              CALL RDPSF(IPSFO,NSHI,ISEC,KODE)
              IF(KODE.EQ.-1) THEN
                GO TO 94
              ELSE
                RWR=RWR+1.0D0
...
C          Z=SZ0
C Fuente CCA
C Abrimos archivo para guardar posiciones iniciales X, Y, Z
C           OPEN(99,FILE='xyz.dat')

           ZZ = 0.0D0
  955      IF(ZZ.GT.-0.84346D0) THEN
               ZZ = -1.0D0 + 2.0D0 * RAND(1.0D0)
               GO TO 955
           ENDIF
           T = RAND(1.0D0) * 2.0D0 * 3.1415926535897932D0
           R = SQRT(1.0D0 - ZZ*ZZ)
           XX = R * COS(T)
           YY = R * SIN(T)
           X = XX * 1.21D0     ! Rc + 0.1 mm = Rc + 0.01 cm
           Y = YY * 1.21D0
           Z = ZZ * 1.21D0 + 1.2D0   ! ZZ * (Rc+0.01cm) + Rc

C           WRITE(99,'(1X,1P,3E14.6)') X,Y,Z
...
C
 ...
 9425   FORMAT(/1X,'#  columns 1 to 3: coordinates X,Y,Z of the bin',
     1    ' centers',/1X,'#  4th column: dose (eV/g).',
...
C        Modificado para obtener perfiles a varias profundidades

C ***** bloque a copiar para cada perfil.
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa
C
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
...
C
 ...
 9440   FORMAT(
     ...
     1      XYZ,MAX(YAV,1.0D-35),MAX(YERR,1.0D-35)
        ENDDO
        CLOSE(9)
C *** fin del bloque de perfil

C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=1mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C       Hay n£mero impar de puntos en cada perfil, centrados en 0.

        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 2 = 1 mm de profundidad
        I3C=(NDB(3)/2)+1+2
C
        OPEN(9,FILE='x-dose_1mm.dat')
          WRITE(9,9441)
 9441   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=1mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=2mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C       Hay n£mero impar de puntos en cada perfil, centrados en 0.
        ...
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 4 = 2 mm de profundidad
        I3C=(NDB(3)/2)+1+4
C
        OPEN(9,FILE='x-dose_2mm.dat')
          WRITE(9,9442)
 9442   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=2mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=3mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C       Hay n£mero impar de puntos en cada perfil, centrados en 0.
        ...
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 6 = 3 mm de profundidad
        I3C=(NDB(3)/2)+1+6
C
        OPEN(9,FILE='x-dose_3mm.dat')
          WRITE(9,9443)
 9443   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=3mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=4mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 8 = 4 mm de profundidad
        I3C=(NDB(3)/2)+1+8
C
        OPEN(9,FILE='x-dose_4mm.dat')
          WRITE(9,9444)
 9444   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=4 mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=6mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 12 = 6 mm de profundidad
        I3C=(NDB(3)/2)+1+12
C
        OPEN(9,FILE='x-dose_6mm.dat')
          WRITE(9,9445)
 9445   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=6 mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C ***** bloque a copiar para cada perfil. PERFIL en X, Y=0, Z=8mm
C       I1C= eje X, I2C=eje Y, I23=eje Z
C       X, Y ejes tranversales a la placa, Z eje perpendicular al centro
C       de la placa. Se asume que puntos del perfil est n cada 0.5 mm
C
        I1C=(NDB(1)/2)+1
        I2C=(NDB(2)/2)+1
C       perfil seg£n eje X a Z=0.5 x 16 = 8 mm de profundidad
        I3C=(NDB(3)/2)+1+16
C
        OPEN(9,FILE='x-dose_8mm.dat')
        WRITE(9,9446)
 9446   FORMAT(
     1     1X,'#  Results from PENMAIN.',
     1    /1X,'#  Dose distribution along the central X axis, Z=8 mm',
     1    /1X,'#  1st column: x (cm).',
     1    /1X,'#  2nd column: dose (eV/g).',
     1    /1X,'#  3rd column: statistical uncertainty (3 sigma).',/)
        DO I1=1,NDB(1)
...
C                 When ICLOSE=1, particles remaining in the buffer are
C                 transferred to the psf, and the output unit is closed.
C                 If ICLOSE=-1, particles are moved to the psf, but the
